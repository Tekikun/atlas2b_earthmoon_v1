...
C NO OTHER NAME IS MORE APPROPRIATED...
C IS THE OLD ATLAS.F SUPERUPDATED BECAUSE CALCULATES WIDTHS AND LIBRATION
C PERIODS CONSIDERING PLANETARY ECCENTRICITIES
C REFERENCE: GALLARDO 2020, CMDA
...
C CAN BE CHANGED
      ...
      COMMON XM0,XSE,XEC,XMA,RH,rhtol
...
C INPUT
      OPEN(1,FILE='superatlasv3.inp',STATUS='old')
      read (1,*)  zcomment
C MASS CENTRAL STAR
      read(1,*) xm0
      read (1,*)  zcomment
...
C i,argper and ome is longitude of the node asuming that the perihelion
C of the planet is at the origin
C READ initial e, final e, delta e for calculations
      read (1,*)  zcomment
      read (1,*) exci,excf,dele
      

      read (1,*)  zcomment
C TOLERANCE FACTOR FOR CALCULATING WIDTH
      read(1,*) RHTOL
      close (1)
C PLANETARY HILL RADIUS
      ...
      WRITE(*,*)'------------------------------------------------------'
      WRITE(*,*)'SUPERATLASV3 OF MEAN MOTIONS RESONANCES kp:k'
      ...
      WRITE(*,*)'of given planets described in superatlasv3.inp'
      WRITE(*,*)'Output: superatlasv3.out'
...
C OK. WE NEED TO CALCULATE  NCONT RESONANCES
      OPEN(1,FILE='superatlasv3.out',STATUS='UNKNOWN',ACCESS='APPEND')
      WRITE(1,*)'I WILL CALCULATE ',NCONT,' RESONANCES',cabsigma
      write(1,*) cabezal4,"RHTol=",rhtol
      DO I=1,NCONT

      write(*,*)"calculating ",i,"/",ncont
...
C ANCHO2 IS RESONANCE FULL stable WIDTH
 111     ANCHO2=8.D0/3.D0/(KG2*MST)*(DELR)*A2**3
     ...
     *TLIB(1),TLIB(2),TLIB(3),TLIB(4)
      WRITE(*,*)INPLA,KP,KK,SALSEM,exc,ANCHO2
